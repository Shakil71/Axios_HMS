...
export interface DirectoryRefs {
  countries: Record<string, { id: string; cityId: string }>;
  hospitals: Record<string, string>; // country name -> hospital id
  treatments: Record<string, string>; // treatment name -> id
  specialties: Record<string, string>;
}

/**
 * Sample directory: countries, cities, hospitals, treatments, FICTIONAL doctors (flagged isDemo), FAQs and public settings.
 * Idempotent (upserts by slug); the sample doctors are recreated on every run.
 */
export async function seedDirectory(prisma: PrismaClient): Promise<DirectoryRefs> {
  ...
  for (const d of doctorDefs) {
    ...
    await prisma.doctor.create({
      data: {
        ...
        status: 'PUBLISHED', isVerified: true, isDemo: true, verifiedAt: new Date(), isFeatured: true,
  ...
  return { countries, hospitals, treatments, specialties };
}
